...
async function updateWishlistWithLatest(wishlist) {
  ...
  const updatedWishlist = await Promise.all(
    wishlist.map(async (wishlistObj) => {
        ...
        wishlistObj.snapshot.price = variant.isSale ? variant.salePrice : variant.price;
...
exports.getWishlists = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(200)
    .json({ status: "success", result: updateWishlist.length, data: updateWishlist });
});
...
exports.addToWishlist = asyncHandler(async (req, res, next) => {
    ...
    const isExists = user.wishlist.find((item)=> item.product.toString() === productId && item.variant.toString() === variantId)
    if(isExists){
