...
const {
  setSlug,
  isRefBelongsToModel,
  isPriceAfterDiscountLess,
  isArrayOfRefBelongsToModel,
  isSpecificModelBelongsToModel,
  checkMaxImages,
...
exports.createProductValidator = [
  ...
  check("priceAfterDiscount")
    .optional()
    .isFloat()
    .withMessage("priceAfterDiscount is number")
  ...
  check("subCategories")
    ...
    .isArray().withMessage("subcategories is array")
    .custom((subcategoriesReceived, { req }) =>
      isArrayOfRefBelongsToModel(subcategoriesReceived, req, SubCategory)
    )
    .custom(async (subcategoriesReceived, { req }) =>
      isSpecificModelBelongsToModel(subcategoriesReceived, req, SubCategory, {
        category: req.body.category,
      })
    ),
    check("subCategories.*").isMongoId().withMessage("Invalid subCategory id format"),
  ...
  check("brand")
    ...
    check("imageCover").custom((val,{req})=>checkMaxImages(val,req)),
  validatorMiddleware,
];
...
exports.updatedProductValidator = [
  ...
  check("description")
    ...
    .isLength({ max: 500 })
    .withMessage("too long product description"),
  check("count").isInt().withMessage("count is number").optional(),
  check("sold").optional().isInt().withMessage("sold is number"),
  check("price")
    .isFloat({ max: 20000 })
    .withMessage("price must be less than 20000")
  ...
  check("subCategories")
    ...
    check("subCategories.*").isMongoId().withMessage("Invalid subCategory id format").optional(),
  check("brand")
    .optional()
    .isMongoId()
    .withMessage("Invalid brand id format")
